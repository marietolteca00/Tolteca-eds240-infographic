...
hero_kd_summary <- clean_season %>%
  ...
  filter(games >= 1)   # Only heroes with 3+ games
...
unique(map_hero_performance$hero_name)
unique(hero_kd_summary$hero_name)
...
ggplot(season_progression, 
  ...
  labs(
    x = "Season",
    y = "Win Rate",
    title = "Win Rate Across Seasons by Mode",
    fill = "Mode"
  )
...
hero_polar %>%
  ...
  geom_text(
    data = . %>% filter(value > 0),
    aes(x = 5.2, y = y_pos, label = round(value, 1)),
    size = 3, fontface = "bold"
  ) +
facet_wrap(~ role + hero_name, ncol = 4)+
scale_fill_manual(values = c("Kills" = "#4E5472", "Deaths" = "#99381E")) +
  coord_polar("y") +
  xlim(0, 6) +
  labs(
    fill = "",
